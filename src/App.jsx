...
export default function App() {
  ...
  const cargarOrdenes = async () => {
    ...
    try {
      ...
      if (!res) return; // Si redirigi� por InteractionRequiredAuthError
      if (!res.ok) {
        throw new Error(`HTTP ${res.status}: ${res.statusText || "Error en la petici�n"}`);
      }
      ...
      if (Array.isArray(data)) {
        ...
        throw new Error("El formato de respuesta no es un arreglo v�lido");
      }
    } catch (err) {
  ...
  const handleLogin = async () => {
    try {
      ...
      setGlobalError(err.message || "No se pudo iniciar sesi�n");
    }
  };
  ...
  const handleLogout = async () => {
    try {
      ...
      setGlobalError(err.message || "Error al cerrar sesi�n");
    }
  };
  ...
  const obtenerNombreUsuario = () => {
    ...
    return claims.name || claims.preferred_username || claims.email || claims.given_name || account.name || account.username || "T�cnico";
  };
          ...
          <main className="portal-main">
            <div className="content-card">
              <div className="section-title-row">
                <div>
                  <h3 className="section-title">�RDENES ASIGNADAS</h3>
                  <p className="section-subtitle">Datos consumidos desde API Gateway + AWS Lambda</p>
                </div>
                <div className="actions-header">
                  <button className="btn-reload" onClick={cargarOrdenes} title="Recargar �rdenes">
                    ?? Recargar
                  </button>
                </div>
              </div>
              ...
              {estadoCarga === "cargando" && (
                <div className="state-box loading-box">
                  <div className="spinner"></div>
                  <p>Cargando �rdenes de trabajo desde la API...</p>
                </div>
              )}
              ...
              {estadoCarga === "error" && (
                <div className="state-box error-box">
                  ...
                  <h4>Error al obtener �rdenes de trabajo</h4>
                  <p className="error-msg-detail">{errorDetalle || "No se pudo comunicar con el endpoint de API Gateway."}</p>
                  <p className="error-hint">
                    Verifica que tu API en AWS est� activa, que la ruta <code>/ordenes</code> tenga la integraci�n Lambda adjunta y que CORS est� configurado.
                  </p>
                ...
                <div className="orders-table-wrapper">
                  {ordenes.length === 0 ? (
                    <div className="empty-box">
                      <p>No hay �rdenes de trabajo disponibles en este momento.</p>
                    </div>
                  ) : (
            ...
            <div className="identity-card">
              <details className="claims-inspector">
                <summary className="claims-summary">
                  ?? Informaci�n del ID Token (Claims recibidos de Entra External ID)
                </summary>
        ...
        <div className="center-container">
          <div className="card access-card">
            <div className="brand-header">
              ...
              <p className="portal-subtitle">Portal de t�cnicos</p>
            </div>
            <div className="card-body">
              <p className="access-info">
                Bienvenido al sistema de gesti�n de taller. Por favor, autent�cate con tu cuenta para acceder a tus �rdenes de trabajo asignadas.
              </p>
